...
class BaselineConfig:
    ...
    validation_start: datetime = datetime(2026, 2, 1, tzinfo=timezone.utc)
    validation_end_exclusive: datetime = datetime(2026, 3, 1, tzinfo=timezone.utc)
    ...
    def iter_days(self, split: str = "all"):
        if split == "train":
            start = self.train_start
            end = self.validation_start
        elif split == "validation":
            start = self.validation_start
            end = self.validation_end_exclusive
        elif split == "all":
            start = self.train_start
            end = self.train_end_exclusive
        else:
